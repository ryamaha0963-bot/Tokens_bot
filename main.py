...
# Set Playwright to use system Chrome (if not already set)
...
async def launch_browser(playwright):
    # Explicitly set executable path to system Chrome
...
async def github_signup(page, email, username, password, chat_id, send_fn):
    ...
    for attempt in range(max_retries + 1):
        try:
            if attempt > 0:
                await send_fn(f"🔄 Retry {attempt}/{max_retries}... Reloading page.")
            ...
            # ----- Email field -----
            await send_fn("✏️ Email field dhundh raha hoon...")
            email_selectors = [
            ...
            for sel in email_selectors:
                try:
                    ...
                    continue
            if not email_found:
                ss_path = f"{SCREENSHOT_DIR}/debug_{chat_id}_{int(time.time())}.png"
                await page.screenshot(path=ss_path)
                await send_fn(f"⚠️ Email field nahi mila. Page title: {await page.title()}")
                return {"status": "error", "message": "Email field not found. Check screenshot.", "screenshot": ss_path}
            await page.wait_for_timeout(1000)

            # ----- Password field -----
            await send_fn("🔑 Password field dhundh raha hoon...")
            pwd_selectors = [
            ...
            # ----- Username field -----
            await send_fn("👤 Username field dhundh raha hoon...")
            user_selectors = [
            ...
            if "captcha" in content.lower() or await page.locator("iframe").count() > 0:
                ss_path = f"{SCREENSHOT_DIR}/captcha_{chat_id}_{int(time.time())}.png"
                await page.screenshot(path=ss_path)
                return {"status": "captcha", "url": page.url, "screenshot": ss_path}

            # If we're on a page with "Dashboard" or "Settings", consider success
